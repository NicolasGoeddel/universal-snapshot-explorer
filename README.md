...
[![Build and Push Docker Image](https://github.com/NicolasGoeddel/zfs-snapshot-explorer/actions/workflows/docker-publish.yml/badge.svg)](https://github.com/NicolasGoeddel/zfs-snapshot-explorer/actions/workflows/docker-publish.yml)
[![GitHub Packages](https://img.shields.io/badge/Container_Registry-GHCR-blue.svg?logo=docker&logoColor=white)](https://github.com/NicolasGoeddel/zfs-snapshot-explorer/pkgs/container/zfs-snapshot-explorer)
...
A high-performance, lightweight, read-only web explorer and audit tool for **OpenZFS snapshots**.

Instead of browsing snapshots one by one or mounting `.zfs/snapshot` manually over SSH, **ZFS Snapshot Explorer** visualizes the complete version history of all files and folders across all available snapshots simultaneously.
...
services:
  zfs-snapshot-explorer:
    image: ghcr.io/nicolasgoeddel/zfs-snapshot-explorer:latest
    container_name: zfs-snapshot-explorer
...
# Optional: Manually define custom roots or backup datasets
roots:
  Backup Rocky:
    # Absolute path to the ZFS dataset root directory (must contain the .zfs folder)
    root_path: /host/mnt/data/backups

    # Subfolder inside the ZFS dataset root. Useful if the dataset does daily snapshots at
    # the root level but contains nested home folders or root partition mirrors (like 'Rocky')
    sub_path: Rocky

    # Target-specific passwd/group files to map UIDs/GIDs of this specific backup partition.
    # If omitted, falls back to raw numerical UIDs/GIDs for files inside this root.
    user_map: etc/passwd
    group_map: etc/group

    # Snapshot patterns specifically for this root. For more details see above.
    snapshot_patterns:
      - "rocky-backup-%Y-%m-%d"
...
2. **Start development server (with hot-reload):**
   ```sh
   uv run zfsse --reload --port 8000 --config-file "examples/truenas/config.yaml"
   ```
